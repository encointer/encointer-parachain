...
function print_balances() {
    ...
    echo "Encointer ROC"
    polkadot-js-api --ws $ENCOINTER query.assets.account 0 $1 | jq .account.balance
}

echo "*** total supply of ROC@Encointer"
polkadot-js-api --ws $ENCOINTER query.balances.totalIssuance | jq .totalIssuance
echo "*** ROC Treasury@Encointer balance"
polkadot-js-api --ws $ENCOINTER query.assets.account 0 $TREASURY | jq .account.balance
